import { Check, Copy, ListTree, Star } from 'lucide-react'
import { useState } from 'react'

import AssistantToolLogsDrawer from '@/features/chat/components/assistant-tool-logs-drawer'
...
function AssistantMessageActionBar({
  ...
  const [toolLogsDrawerOpen, setToolLogsDrawerOpen] = useState(false)
    ...
    <>
      <div className={cn('inline-flex items-center gap-0.5 text-[#95A0B0]', className)}>
        <Button
          ...
          size="icon"
          aria-label={
            copied ? t('chat.assistantAction.replyCopied') : t('chat.assistantAction.copyReply')
          }
          title={
            copyError ??
            (copied ? t('chat.assistantAction.replyCopied') : t('chat.assistantAction.copyReply'))
        ...
        <Button
          ...
          aria-label={
            favorited
              ? t('chat.assistantAction.replyFavorited')
              : t('chat.assistantAction.favoriteReply')
          }
          title={
            favorited
              ? t('chat.assistantAction.replyFavorited')
        ...
        <Button
          ...
          variant="ghost"
          size="icon"
          aria-label={t('chat.assistantAction.openToolLogs')}
          title={t('chat.assistantAction.openToolLogs')}
          className="size-[1.4rem] rounded-[0.45rem] text-current hover:bg-secondary/80 hover:text-foreground"
          onClick={() => {
            setToolLogsDrawerOpen(true)
          }}
        >
          <ListTree className="size-[0.9rem]" />
        </Button>
      </div>

      <AssistantToolLogsDrawer
        open={toolLogsDrawerOpen}
        toolLogs={toolLogs ?? []}
        onClose={() => setToolLogsDrawerOpen(false)}
      />
    </>
  )
}
