...
import { stripAssistantInternalScaffolding } from '../renderer/src/shared/text/assistant-visible-text'
...
describe('stripAssistantInternalScaffolding', () => {
  ...
  it('supports relevant_memories tag variants', () => {
    const input = ['<relevant_memories>', 'Internal memory note', '</relevant_memories>', 'Visible'].join('\n')
    expect(stripAssistantInternalScaffolding(input)).toBe('Visible')
  })
